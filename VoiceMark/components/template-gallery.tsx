    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${selectedCategory === null
              ...
              : 'bg-background/70 text-muted-foreground hover:bg-muted/80'
            }`}
        >
          All
        </button>
        {categories.map((cat) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors capitalize ${selectedCategory === cat
                ? 'bg-primary text-primary-foreground'
                : 'bg-background/70 text-muted-foreground hover:bg-muted/80'
              }`}
          >
      ...
      <div className="space-y-2">
        {filteredTemplates.map((template) => (
          <div
            key={template.id}
            className="rounded-xl border border-border/60 bg-card/60 overflow-hidden hover:bg-card/70 transition-colors"
          >
            <button
            ...
            {expanded === template.id && (
              <div className="border-t border-border/40 px-4 py-3 bg-background/60 space-y-3">
                {/* Template script */}
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-muted-foreground uppercase">Template</p>
                  <div className="bg-background/80 rounded p-3 text-xs leading-relaxed text-foreground max-h-40 overflow-y-auto font-mono">
                    {template.script}
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 text-xs">
                  <span className="px-2 py-1 rounded-full bg-background/70 text-muted-foreground capitalize">
                    {template.difficulty}
                  </span>
                  <span className="px-2 py-1 rounded-full bg-background/70 text-muted-foreground">
