  ...
  return (
    <main className="min-h-screen bg-background overflow-hidden">
      ...
      <div className="pointer-events-none fixed inset-0">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_15%_10%,rgba(255,106,90,0.12),transparent_40%),radial-gradient(circle_at_85%_0%,rgba(255,190,160,0.12),transparent_38%),linear-gradient(to_bottom,var(--background),color-mix(in_oklch,var(--background)_94%,white))]" />
      ...
      <header className="sticky top-0 z-20 border-b border-border/50 bg-background/70 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5">
          ...
          <Link href="/mode" className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:opacity-90 transition-opacity btn-press">
            Start Practising
      ...
      <div className="fixed bottom-5 left-5 z-10 flex items-center gap-2 rounded-full border border-border bg-card/70 px-3 py-2 text-xs font-semibold text-muted-foreground backdrop-blur-sm">
        <Lock className="h-3 w-3" />
        Private session
      </div>

      {/* Hero Section */}
      <section className="relative mx-auto max-w-6xl px-5 py-16 md:py-24 reveal-on-scroll">
        <div className="space-y-6 text-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-border/50 bg-card/80 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.22em] text-muted-foreground enter-fade" style={{ animationDelay: '40ms' }}>
            <Shield className="h-3.5 w-3.5 text-success" />
            Privacy first
          </div>

          <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight text-foreground font-display title-balance hero-breathe">
            The speaking coach<br />
            <span className="typewriter text-transparent bg-clip-text bg-gradient-to-r from-primary via-primary/80 to-accent">that forgets everything</span>
          </h1>

          <p className="max-w-3xl mx-auto text-lg md:text-xl text-muted-foreground leading-relaxed copy-reveal" style={{ animationDelay: '80ms' }}>
            Practice your speech, get AI feedback, then it's gone. No account, no recordings saved, no trace.
          </p>

          <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4 enter-up" style={{ animationDelay: '120ms' }}>
            <Link href="/mode" className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-6 py-3 font-semibold text-primary-foreground hover:opacity-90 transition-opacity cta-pulse btn-press">
              Start Practising
              <ArrowRight className="h-4 w-4" />
            </Link>
            <Link href="/how-it-works" className="inline-flex items-center justify-center gap-2 rounded-full border border-border bg-card/70 px-6 py-3 font-semibold text-foreground hover:bg-muted/60 transition-colors btn-press">
              See How It Works
            </Link>
          </div>
        </div>
      ...
      {/* Flow Steps */}
      <section className="relative mx-auto max-w-6xl px-5 py-12 reveal-on-scroll">
        <div className="rounded-3xl border border-border bg-card/60 p-8 backdrop-blur-sm motion-panel">
          <div className="motion-sheen" />
          ...
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {FLOW_STEPS.map((step, idx) => (
              <Link
                key={idx}
                href={['', '/mode', '/script', '/studio', '/coach'][idx]}
                className="group rounded-2xl border border-border bg-background/80 p-6 hover:border-primary/50 hover:bg-background transition-all hover-lift motion-item"
      ...
      <section className="relative mx-auto max-w-6xl px-5 py-12 reveal-on-scroll">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {FEATURE_CARDS.map((feature, idx) => (
            <div key={idx} className="rounded-3xl border border-border bg-card/70 p-8 hover-lift motion-in" style={{ animationDelay: `${idx * 90 + 60}ms` }}>
      ...
      <section className="relative mx-auto max-w-6xl px-5 py-16 reveal-on-scroll">
        <div className="rounded-3xl border border-border bg-gradient-to-br from-primary/8 via-background to-accent/12 p-8 md:p-12 motion-panel">
          <div className="motion-sheen" />
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {PRIVACY_FEATURES.map((feature, idx) => (
              <div key={idx} className="space-y-4 motion-in" style={{ animationDelay: `${idx * 100 + 80}ms` }}>
      ...
      <section className="relative mx-auto max-w-6xl px-5 py-12 reveal-on-scroll">
        ...
        <div className="space-y-4">
          {WHO_ITS_FOR.map((item, idx) => (
            <div key={idx} className="rounded-2xl border border-border bg-card/60 p-6 flex items-center gap-4 motion-in" style={{ animationDelay: `${idx * 70 + 60}ms` }}>
